(function($){
  $.fn.limitTextarea = function(opts){
	  ...
	  this.each(function(){
		  ...
		  var info = '<div class="lt_info formtip">还可以输入<b style="color:green;">'+(option.maxNumber- _this.val().length)+'</b>字'+bytestr+'</div>';
		  var fn = function(){
			...
			if(extraNumber>=0){
			  $info.html('还可以输入<b style="color:green;">'+extraNumber+'</b>字'+bytestr);	
			  option.onOk(_this);
			}
			else{
			  $info.html('已经超出<b style="color:red;">'+(-extraNumber)+'</b>字'+bytestr); 
			  option.onOver(_this);
			}  
		  };
