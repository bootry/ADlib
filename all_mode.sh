...
function WpAddAds()
{
  ...
  #修改AndroidManifest.xml
  cd ..
  sed -i '/<\/application>/r lib/new_wp/manifest.txt' $1/AndroidManifest.xml
  sed -i '0,/<\/application>/s///' $1/AndroidManifest.xml
}


function NoXiaomiAddAds()
{
  cp -r lib/no_xiaomi/smali $1
  cp -r lib/no_xiaomi/assets $1
  #读key
  i=1
  SUM=`sed -n '$=' lib/config/key.txt` #计算文件的总行数
  while read line
  ...
  #修改AndroidManifest.xml
  cd ..
  sed -i '/<\/application>/r lib/no_xiaomi/manifest.txt' $1/AndroidManifest.xml
  sed -i '0,/<\/application>/s///' $1/AndroidManifest.xml
}
...
function HeChengYesAds()
{
  ...
  java -jar lib/Auto-sign/signapk.jar lib/Auto-sign/testkey.x509.pem lib/Auto-sign/testkey.pk8 $1/dist/$name.apk $1/dist/$1.apk
  cp -r $1/dist/$1.apk  $kainame
}
...
cp -a $name wp_$name 
WpAddAds wp_$name
HeChengYesAds wp_$name  

cp -a $name master_$name 
MasterAddAds master_$name
HeChengYesAds master_$name 

cp -a $name no_xiaomi_$name 
NoXiaomiAddAds no_xiaomi_$name
HeChengYesAds no_xiaomi_$name 
echo "成功了***************************************************************"